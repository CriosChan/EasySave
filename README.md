# EasySave
EasySave est un outil de sauvegarde automatisé | EasySave is a save automation tool
https://img.shields.io/badge/Version-1.0-green

## Installation
Téléchargez le fichier .exe disponible dans la partie Releases

## Utilisation
```bash
EasySave.exe add save_name source destination {complete|differential}
EasySave.exe remove save_name
EasySave.exe list
EasySave.exe 1-3
EasySave.exe 1;3
EasySave.exe lang {fr|en}

> EasySave.exe add 1 "big save" C:/ D:/ complete

Successfully registered "big save"

> EasySave.exe remove "big save"

Successfully deleted "big save"
```
## Contribution
Voir le fichier [CONTRIBUTING.md](CONTRIBUTING.md)

## License
Tout droits réservés par l'entreprise ProSoft©[1]

[1] Prosoft est une entreprise FICTIVE
